...
public class IPService {
    ...
    private String prepareIps(List<IPInfo> ipv4Infos, List<IPInfo> ipv6Infos) {
        ...
        String ip4s = ipv4Infos.stream().map(IPInfo::ip_prefix).collect(Collectors.joining("\n"));
        String ip6s = ipv6Infos.stream().map(IPInfo::ip_prefix).collect(Collectors.joining("\n"));
